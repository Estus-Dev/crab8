...
impl Debug for Instruction {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let disassembly = match self {
            ClearScreen => "clear".to_owned(),
            Return => "return".to_owned(),
            Jump(addr) => format!("jump {addr:#03X}"),
            Call(addr) => format!("call {addr:#03X}"),
            IfNot(r, value) => format!("if {r} != {value:#02X}"),
            If(r, value) => format!("if {r} == {value:#02X}"),
            IfNotRegs(r1, r2) => format!("if {r1} != {r2}"),
            Store(r, value) => format!("{r} := {value:#02X}"),
            Add(r, value) => format!("{r} += {value:#02X}"),
            Copy(r1, r2) => format!("{r1} := {r2}"),
            Or(r1, r2) => format!("{r1} |= {r2}"),
            And(r1, r2) => format!("{r1} &= {r2}"),
            Xor(r1, r2) => format!("{r1} ^= {r2}"),
            AddReg(r1, r2) => format!("{r1} += {r2}"),
            SubReg(r1, r2) => format!("{r1} -= {r2}"),
            ShiftRight(r1, r2) => format!("{r1} >>= {r2}"),
            ShiftLeft(r1, r2) => format!("{r1} <<= {r2}"),
            SubFromReg(r1, r2) => format!("{r1} =- {r2}"),
            IfRegs(r1, r2) => format!("if {r1} == {r2}"),
            StoreAddress(addr) => format!("i := {addr:#03X}"),
            JumpOffset(addr) => format!("jump0  {addr:#03X}"),
            Rand(r, mask) => format!("{r} := random {mask:#02X}"),
            Draw(r1, r2, rows) => format!("sprite {r1} {r2} {rows}"),
            IfNotPressed(key) => format!("if {key} -key"),
            IfPressed(key) => format!("if {key} key"),
            ReadDelay(r) => format!("{r} := delay"),
            ReadInput(r) => format!("{r} := key"),
            SetDelay(r) => format!("delay := {r}"),
            SetSound(r) => format!("buzzer := {r}"),
            AddAddress(r) => format!("i += {r}"),
            LoadSprite(r) => format!("i := hex {r}"),
            WriteDecimal(r) => format!("bcd {r}"),
            Write(r) => format!("save {r}"),
            Read(r) => format!("load {r}"),
            Nop(instruction) => format!("nop {instruction:#04X}"),
        };
